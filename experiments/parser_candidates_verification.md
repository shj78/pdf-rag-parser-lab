...
### 9-3. OpenDataLoader hybrid 모드 — **시험 완료**

`opendataloader-pdf[hybrid]` 를 격리 venv `.venv-opendataloader/` 에 설치하고, `docling-fast` backend 로 안내책자 1~3페이지 smoke 를 실행했다.
...
- local 모드: 1~3페이지 정상 종료, `pages=3`, `text_blocks=0`, `warnings=0` — §9-1 과 동일하게 텍스트 레이어 없는 PDF 에서는 본문 복원 안 됨.
- hybrid `docling-fast/full` 모드: 1~3페이지 정상 종료, `pages=3`, `text_blocks=248`, `warnings=0`, `chars=947`.
- 서버 측 변환 시간: 약 69.8초 (DocumentConverter 초기화 이후 3페이지 처리).
- backend health check: `/docs`, `/health` 모두 200 응답.

해석:

- OpenDataLoader 도 OCR 을 켜면 한국어 텍스트 복원은 가능하다.
- 다만 hybrid 모드는 별도 backend 서버를 띄워야 하고, 어댑터는 health check 만 수행한다.
- 현재 어댑터는 paragraph/heading 중심으로 정규화하며, 표는 `TableBlock` 으로 보존하지 못한다. 안내책자형 baseline 은 여전히 MinerU 가 우위.
- `hancom-ai` backend 와 `--table-method cluster` 는 아직 별도 미시험.
...
| hybrid | 있음 (별도 백엔드 서버) | ✅ 했음 (§9-3) | 텍스트 복원 가능, 표는 paragraph 로 파편화 |
...
- "OCR 켠 OpenDataLoader (docling-fast) vs MinerU" 의 최소 smoke 는 완료됐다.
- 그러나 OpenDataLoader `hancom-ai` backend 와 table 개선 옵션은 미시험이라, 전체 품질 비교는 아직 닫히지 않았다.
- **어댑터 복잡도** (MinerU 단일 패키지 vs OpenDataLoader hybrid 서버 라이프사이클) 차이는 §9-4 결론에 그대로 유효.
- 즉 "Phase 1 = MinerU 우선" 결정은 품질뿐 아니라 **운영 복잡도** 근거까지 포함한다.

---

### 9-7. 35페이지 풀변환 실측 (2026-05-21 — 완료)

**초기 실패 → 재시도 성공**. 5/21 첫 시도(어댑터 경유)는 `Error: 1 task(s) failed while processing documents` 로 fail (stderr 잘림 + `resource_tracker: 6 leaked semaphore objects`). 후속 재현 시도에서:

| 시도 | 호출 경로 | 페이지 | 결과 |
| --- | --- | --- | --- |
| ① | 어댑터 (tempdir + subprocess) | 0–34 (전체) | fail (task#1 fail) |
| ② | CLI 직접 (`-s 0 -e 9`) | 0–9 | success (exit 0) |
| ③ | CLI 직접 (페이지 옵션 없음) | 0–34 (전체) | success (exit 0) |

→ **단발성 실패 결론**. 페이지 수 한계도, 어댑터 결함도 아님. mineru 의 mp 워커가 가끔 hang/fail 하는 것으로 보임 (재시도 정책 검토 가능).

**③ 실측치** (CPU pipeline, language korean, Apple Silicon):

- 처리 시간: 10분 13초 (모델 init 약 11.5초 1회성 포함, ≈14 s/page).
- 출력 (`*_content_list.json`): **711 items** — text 516, table 32, image 53, chart 7, header 9, aside_text 69, footer 2, page_number 23.
- 표 보존: HTML `<table>` + `rowspan/colspan`. 달력 표·일정표 모두 추출.
- 한국어 OCR: 정상. 알려진 오인식 (`1회차→1회찬`, `마감→마각`) 일부 잔존.
- 헤딩 자동 부여: `## 4월`, `# 이거모르면큰일나!` 등.

**baseline 자격**: 확정. 청킹·평가 실험 입력으로 사용 가능.

운영 메모:
- 어댑터 입장에서 단발성 실패에 대비해 1회 재시도 옵션을 향후 추가할 수 있음 (현재는 즉시 RuntimeError raise).
- 35페이지 결과 보존 위치: `/tmp/mineru-full-out/` — `/tmp` 휘발성, 영구화 필요 시 `data/` 이동 검토.
...
**시험 환경 상태**:
- `.venv-mineru/` — MinerU 격리 venv. `PATH=".venv-mineru/bin:$PATH"` 로 CLI 호출.
- `.venv-opendataloader/` — OpenDataLoader 격리 venv. hybrid backend 실행 파일 포함.
- `~/.cache/huggingface/hub/models--opendatalab--PDF-Extract-Kit-1.0` — 1.1 GB 영구 캐시. **삭제 비권장** (다음 세션에서 재사용).
- OpenDataLoader hybrid backend 는 장기 실행 프로세스라 smoke 후 종료해야 한다.
- 출력 샘플은 `/tmp` 기반 임시 산출물이며 영구화하지 않았다.

**다음 세션 첫 액션 후보**:
1. OpenDataLoader `hancom-ai` backend 또는 `--table-method cluster` 시험 여부 결정.
2. 파서 비교 결과를 downstream 으로 넘길 `FixedSizeChunker` 구현.
3. retrieval/evaluation 실험을 닫기 위한 in-memory index 와 NDCG@k 구현.
4. Phase 3-1 의 데이터셋 relevance 라벨링 방법 결정.
